...
def summarize_chunk(text: str) -> str:
    ...
    """
    # Fallback to extractive sum if no combined method is used
    # if OPENAI_API_KEY:
    #     try:
...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    """
    # Fallback if no combined method is used
    # if OPENAI_API_KEY:
    #     try:
    #         from openai import OpenAI
    #         client = OpenAI()
    ...
    if OPENAI_API_KEY:
        try:
            ...
            resp = client.chat.completions.create(
                model=LLM_MODEL,
                messages=[
                    {"role": "system", "content": """Phân tích đoạn văn và trả về ĐÚNG ĐỊNH DẠNG JSON MÀ KHÔNG CÓ THÊM BẤT KỲ VĂN BẢN KHÁC:
{
