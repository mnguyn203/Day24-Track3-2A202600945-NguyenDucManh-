...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # Extract content if response is a dict (newer NeMo versions)
...
if __name__ == "__main__":
    ...
    lat = measure_p95_latency(sample_inputs, n_runs=10)
    print(f"Latency P95 — Presidio: {lat['presidio_ms']['p95']}ms | NeMo: {lat['nemo_ms']['p95']}ms | Total: {lat['total_ms']['p95']}ms")
    print(f"Budget OK (500ms): {lat['latency_budget_ok']}")

    with open("reports/guard_results.json", "w", encoding="utf-8") as f:
        json.dump({"latency": lat, "adversarial_pass_rate": f"{passed}/{len(adversarial_set)}"}, f, ensure_ascii=False, indent=2)
